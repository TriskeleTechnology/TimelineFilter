...
import { textMeasurementService, textUtil, wordBreaker } from "powerbi-visuals-utils-formattingutils";
const LeftPadding = 10;
export function willLabelsFit(axisProperties, availableWidth, textMeasurer, properties) {
...
export function willLabelsWordBreak(axisProperties, margin, availableWidth, textWidthMeasurer, textHeightMeasurer, textTruncator, properties) {
    let labels = axisProperties.values;
    let labelMaxWidth = axisProperties.xLabelMaxWidth !== undefined
        ? axisProperties.xLabelMaxWidth
        : availableWidth / labels.length;
 ...
 */
export function rotate(labelSelection, maxBottomMargin, textTruncator, textProperties, needRotate, needEllipsis, axisProperties, margin, scrollbarVisible) {
    let rotatedLength;
    let defaultRotation;
...
export function wordBreak(text, axisProperties, maxHeight) {
...
export function clip(text, availableWidth, svgEllipsis) {
    if (text.size() === 0) {
        return;
    }
    text.each(function () {
